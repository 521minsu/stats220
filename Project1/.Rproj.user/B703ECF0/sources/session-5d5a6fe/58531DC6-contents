...
# define initial frame with basic info on gif (this makes the first and second frame of the gif to be identical)
frames <- image_read("PartC_img_src/0.png")%>%
...
frames <- image_append(c(annotation, curr_frame), TRUE)
frames
...
for(i in 0:9) {
  ...
  animation <- image_animate(frames, loop = 100)
